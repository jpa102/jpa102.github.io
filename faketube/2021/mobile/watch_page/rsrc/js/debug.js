...
function UrlExists(url) {
	...
	try {
		http.send();
		console.log(`http status for UrlExists: ${http.status}`);
	} catch {
		if (http.status == 404) {
			console.log(`http status for UrlExists: ${http.status}`);
		}
		...
		`https://noembed.com/embed?url=https://www.youtube.com/watch?v=${id}`
			...
			response.json().then((json) => {
				if (json) {
					...
					document.querySelector("#videos-button").href = author_url + "/videos";
					document.querySelector("#about-button").href = author_url + "/about";
				}
			})
		}
	);
...
function AllInOneVideoReplacementOperations(inputvideoid) {
	if (inputvideoid == "") {
		console.log(`[AllInOneVideoReplacementOperations] no video id provided, terminating...`);
		return;
	}
...
function appendVideoIdToUrl() {
	if (window.confirm("are you sure you want to append the video id in the url?") == true) {
		alert(`video id '${global_data.yt.videoId}' will be appended to the url, see the address bar for the result`);
		window.history.replaceState(null, '', `https://jpa102.github.io/faketube/2021/mobile/watch_page/?v=${global_data.yt.videoId}`);
	} else {
		return;
	}
}
...
function printLikePercentage() {
	alert(`like percentage: ${_volatile_votes.likepercentage}% \nrounded like percentage: ${_volatile_votes.roundedlikepercent}% \n\nraw like percentage: ${getPercentage(global_data.ryd_data.rawLikes, global_data.ryd_data.rawDislikes, 1)}% \nrounded raw like percentage: ${getPercentage(global_data.ryd_data.rawLikes, global_data.ryd_data.rawDislikes, 2)}%`);
}

function printAverageRating() {
	alert(`average rating: ${_volatile_votes.averageRating} \nrounded average rating: ${_volatile_votes.roundedRating}`);
}
...
function setLikeDislikeCountsFormatting(formatType) {
	if (formatType == "") {
		console.log(`format types: \n\n\t- compactShort\n\t- compactLong\n\t- standard`);
		return;
	}

	extConfig.numberDisplayFormat = formatType;
}

function getDaysAgo(pastDateString) {
	const pastDate = new Date(pastDateString); // Example: 1995-07-21 - July 21, 1995
	const currentDate = new Date();

	// Reset time components to avoid issues with time differences within a day
...
function getYearsAgo(yearString) {
	const pastDate = new Date(yearString); // Example: 1995-07-21 - July 21, 1995
