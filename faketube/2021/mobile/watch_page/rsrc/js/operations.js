...
function addLikeCount() {
	...
	_volatile_votes.averageRating = getAverageRating(_volatile_votes.likeCount, _volatile_votes.dislikeCount, 5);
	_volatile_votes.roundedRating = getAverageRating(_volatile_votes.likeCount, _volatile_votes.dislikeCount, 6);
	_volatile_votes.likepercentage = getPercentage(_volatile_votes.likeCount, _volatile_votes.dislikeCount, 1);
	_volatile_votes.roundedlikepercent = getPercentage(_volatile_votes.likeCount, _volatile_votes.dislikeCount, 2);
...
function estimateLikeCountFromRYD() {
	if (window.confirm(`by executing this function, you accept that it will use a high dangerous usage of cpu while calculating the estimated like counts\n\nare you sure you want to continue? this may take some time depending on the votes...\n\nactual likes: ${global_data.ryd_data.likeCount.toLocaleString()}, raw likes: ${global_data.ryd_data.rawLikes.toLocaleString()}\nestimated dislikes: ${global_data.ryd_data.dislikeCount.toLocaleString()}, raw dislikes: ${global_data.ryd_data.rawDislikes.toLocaleString()}\n\n\(this is only used to estimate like counts based on rawLikes and rawDislikes from Return YouTube Dislike API, if there's enough votes for them\)`) == true) {
		// detect if the likeCount is NaN in global_data.yt object
		// (like counts set to private by the creator) and artifically set a 0 at _volatile_votes
		if (isNaN(global_data.yt.likeCount) == true && _volatile_votes.averageRating == NaN || isNaN(_volatile_votes.averageRating) == true) {
			_volatile_votes.likeCount = 0;
		} else {
			console.log(`function is not executed because the like counts are not set to private by the creator`);
			return;
		}
		...
		document.querySelector("head").insertAdjacentHTML(
			...
			`
				<style id="experimental-material-you-comments-section-css" type="text/css">
					:root { --video-player-comment-caption-size: 0.78em !important; }
				...
				`
					<div style="width: 17px; height: 17px; margin: 0px 0px 0px 14px;">
						<style id="experimental-material-you-comments-section-migration-phase" type="text/css">
							...
							#comment-caption, #comment-counts { font-size: 0.8em !important; }
				...
				`
					<div id="all-buttons-container" class="exp-all-buttons-container">
						<style id="experimental_material_you_action_buttons" type="text/css">
							...
							.menu-buttons { display: flex; align-items: center; padding: 6px 14px; background-color: rgba(0, 0, 0, 0.1) !important; width: unset; }
