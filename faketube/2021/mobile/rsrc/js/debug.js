		...
		`https://returnyoutubedislikeapi.com/votes?videoId=${id}`
			...
			response.json().then((json) => {
				if (json && !("traceId" in response) && !statsSet) {
					...
					document.querySelector("#video-metadata").innerText = numberFormat(unformattedviews) + " views • " + ReceivedVideoMetadata.dateuploaded;
			...
			metadataurl
				...
				response.json().then((json) => {
					if (json) {
						...
						console.log("\nShort description: " + shortdesc + "\nLong description: " + fulldesc + "\nDate and time uploaded: " + uploaddateandtime + "\nDate uploaded: " + uploaddate + "\nTime uploaded: " + uploadtime + "\nSub counts with 0's: " + subcountintguess + "\nComment count: " + commentCount + "\nProfile picture link: " + ytOnlineProfilePicture + "\nVideo is YouTube Shorts: " + isYouTubeShort);
