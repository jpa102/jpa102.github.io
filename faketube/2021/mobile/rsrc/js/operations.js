...
//	hide the dislike counts and prevent it from showing up
if (faketube.config_.EXPERIMENT_FLAGS.hide_dislike_counts == true) {
	setTimeout(function() {
		...
		document.querySelector("#dislike").insertAdjacentHTML(
			"beforeend",
			`
				<span id="no-dislike-count-renderer" class="menu-buttons-text"></span>
			`
		);
		document.querySelector("#no-dislike-count-renderer").innerText = "Dislike";
	}, 50);
}
...
if (faketube.config_.EXPERIMENT_FLAGS.watch_page_experimental_material_you_comments_section == false && faketube.config_.EXPERIMENT_FLAGS.watch_page_experimental_material_you_comments_section_migration_phase == true) {
	setTimeout(function() {
		// make the comments comemnt_count texts smaller, and the comment box a bit rounded
		document.head.insertAdjacentHTML(
			"beforeend",
			`
		...
		// add a right svg icon beside the comment box
			...
			`
				<div id="all-buttons-container" class="exp-all-buttons-container">
					<style id="experimental_material_you_action_buttons" type="text/css">
						...
						.button-text-style { width: 24px; height: 24px; margin-right: 6px; }
						...
						.menu-buttons-text { display: revert; text-overflow: revert; min-width: unset; }
						.button-border { content: ""; border-left: 1px solid rgba(0,0,0,0.1); padding-left: 14px; }
